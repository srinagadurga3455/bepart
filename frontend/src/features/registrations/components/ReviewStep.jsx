...
import { findCountFieldName, getSelectedCount, getVisibleFields, withDynamicRequired, getMemberGroupIndex } from './memberGroups';

export default function ReviewStep({ formStructure, formData, onBack, onEdit, onConfirm, isSubmitting, confirmLabel, feeAmount }) {
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <List sx={{ mb: 3 }}>
        {sections.map((section, sectionIndex) => (
          <Box key={section.id} sx={{ mb: 3 }}>
            <Typography variant="h6" fontWeight={600} color="text.primary" gutterBottom sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  {(() => {
                    ...
                    return getVisibleFields(section, selectedCount).map((rawField, fieldIndex) => {
                      ...
                      const header = group > 0 && group !== lastGroup ? (
                        <Typography variant="subtitle2" fontWeight={600} color="text.primary" sx={{ mt: fieldIndex === 0 ? 0 : 1 }}>
                          Member {group}
                        </Typography>
                      ) : null;
                      lastGroup = group;
      ...
      {feeAmount !== null && feeAmount !== undefined && (
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2, mb: 3, p: 2, borderRadius: 2, bgcolor: 'grey.100' }}>
          <Typography variant="body1" fontWeight={600}>Registration Fee</Typography>
          <Typography variant="h6" fontWeight={800}>₹{feeAmount}</Typography>
        </Box>
      )}
