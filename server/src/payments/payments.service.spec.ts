...
describe('PaymentsService - Auth ownership & payment flow', () => {
  ...
  it('should allow authenticated student to create payment for own registration', async () => {
    mockPaymentsRepo.findRegistrationWithEvent.mockResolvedValue({
      registrationId: 'reg1',
      phone: '+919876543210',
      event: { id: 1 },
    });

    mockPaymentsRepo.findUserById.mockResolvedValue({
      id: 'user1',
      phone: '+919876543210',
    });
    ...
    expect(mockPaymentsRepo.createPayment).toHaveBeenCalledWith(
  ...
  it('should synchronize Registration.paymentStatus atomically via repo transaction', async () => {
    ...
    mockPaymentsRepo.updatePaymentStatusAtomic.mockImplementation(
    ...
    expect(
      mockPaymentsRepo.updatePaymentStatusAtomic,
  ...
  it('should ensure PATCH /:id/status is ADMIN-only via Roles guard', async () => {
    ...
    const roles = Reflect.getMetadata(
      'roles',
      PaymentsController.prototype.updateStatus,
    );
  ...
  it('should keep PENDING payment without creating Registration (paid flow)', async () => {
    // Simulate pending payment flow:
    // payment has pendingFormData, no registration yet, status PENDING
    mockPaymentsRepo.findPaymentById.mockResolvedValue({
      id: 'payPending',
      status: PaymentStatus.PENDING,
  ...
  it('should create exactly one Registration on PAID via atomic transaction', async () => {
    ...
    expect(res.registration.formData).toEqual({
      branch: 'CSE',
    });
  });
  ...
  it('should rollback transaction if registration creation fails (duplicate)', async () => {
    ...
    mockPaymentsRepo.updatePaymentStatusAtomic.mockRejectedValue(
      new ConflictException(
        'Phone already registered for this event',
      ),
    );
  ...
  it('should rollback if event slots full during PAID', async () => {
    ...
    mockPaymentsRepo.updatePaymentStatusAtomic.mockRejectedValue(
      new BadRequestException('Event slots full'),
    );
  ...
  it('should allow PAID registration with any phone format if canonical matches', async () => {
    // Simulate that payment was created with canonical 9123456789,
    // but lookup with +91 9123456789 should find it.

    // This is tested via registrations service,
    // but also verify payments service stores canonical.
    const { canonicalPhone } = await import('../common/utils/phone');
    ...
    mockPaymentsRepo.findEventById = jest
      .fn()
      .mockResolvedValue({
        id: 17,
    ...
    mockPaymentsRepo.findPendingPaymentByPhoneAndEvent = jest
      .fn()
      .mockResolvedValue(null);

    mockPaymentsRepo.findRegistrationByPhoneAndEvent = jest
      .fn()
      .mockResolvedValue(null);

    mockPaymentsRepo.createPendingPayment = jest
      .fn()
      .mockResolvedValue({
        id: 'pay1',
        phone: '9123456789',
    ...
    // Test via service
    const res = await service.createPending({
      eventId: 17,
      phone: '+91 9123456789',
      amount: 50000,
    } as any);
    ...
    expect(
      mockPaymentsRepo.createPendingPayment,
    ).toHaveBeenCalledWith(
      expect.objectContaining({
        phone: '9123456789',
      }),
